...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		{
			case 'copied':
				WeekSaves.copiedLock = 'unlocked';
				trace('sex');
			case 'cosplay':
				WeekSaves.cosplayLock = 'unlocked';
			case 'lured':
				WeekSaves.luredLock = 'unlocked';
				case 'copied-remix':
					WeekSaves.copiedrLock = 'unlocked';
					trace('sex');
					case 'cookie-beat':
						WeekSaves.cookieLock = 'unlocked';
						case 'new-guest':
							WeekSaves.guestLock = 'unlocked';
							case 'ushanka':
								WeekSaves.ushankaLock = 'unlocked';
								case 'double-trouble':
									WeekSaves.trubluLock = 'unlocked';
								    case 'martian-madness':
										WeekSaves.alienLock = 'unlocked';
										case 'lovely-evening':
											WeekSaves.dateLock = 'unlocked';
											case 'mistletoe':
												WeekSaves.kornerchLock = 'unlocked';
												case 'christmas':
													WeekSaves.wojtichLock = 'unlocked';
		}
		...
		if(SONG.song.toLowerCase() == 'copied')
		{
			introSoundsSuffix = '-korner';
		}
			
		if(SONG.song.toLowerCase() == 'cosplay')
		{
			introSoundsSuffix = '-korner';
		}
			
		if(SONG.song.toLowerCase() == 'lured')
		{
			introSoundsSuffix = '-korner';
		}
			
		if(SONG.song.toLowerCase() == 'copied-remix')
		{
			introSoundsSuffix = '-korner';
		}
			
		if(SONG.song.toLowerCase() == 'cookie-beat')
		{
			introSoundsSuffix = '-korner';
		}
		...
		if(SONG.song.toLowerCase() == 'new-guest')
		{
			introSoundsSuffix = '-wojti';
		}
			
		if(SONG.song.toLowerCase() == 'ushanka')
		{
			introSoundsSuffix = '-wojti';
		}
		...
		if(SONG.song.toLowerCase() == 'martian-madness')
		{
			introSoundsSuffix = '-wojti';
		}
        
		if(SONG.song.toLowerCase() == 'lovely-evening')
		{
			introSoundsSuffix = '-wojti';
		}

		if(SONG.song.toLowerCase() == 'christmas')
		{
			introSoundsSuffix = '-wojti';
		}

		// bruh
		Application.current.window.title = ("Korner\'s Funky Club - "
			+ DefaultSongData.getSongData(StringTools.replace(SONG.song.toLowerCase(), " ", "-"), 'artist')
			+ ' - '
			+ DefaultSongData.getSongData(StringTools.replace(SONG.song.toLowerCase(), " ", "-"), 'name')
		);
		...
		botplayTxt = new FlxText(400, timeBarBG.y + 55, FlxG.width - 800, "WHEN ALIEN MOD UPDATE?", 32);
		...
		callFunc('onCreatePost', []);
		callOnLuas('onCreatePost', []);

		stageBuild.createPost(curStage, boyfriend, gf, dad);
	...
	public function startCountdown():Void
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
			{
				...
				{
					case 0:
						FlxG.sound.play(Paths.sound('intro3' + introSoundsSuffix), 0.9);
					case 1:
						...
						FlxG.sound.play(Paths.sound('intro2' + introSoundsSuffix), 0.9);
					case 2:
						...
						FlxG.sound.play(Paths.sound('intro1' + introSoundsSuffix), 0.9);
					case 3:
						...
						FlxG.sound.play(Paths.sound('introGo' + introSoundsSuffix), 0.9);
					case 4:
				}
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		...
		callFunc('onEvent', [eventName, value1, value2]);
	}
	...
	function noteMiss(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		callFunc('noteMiss', [notes.members.indexOf(daNote), daNote.noteData, daNote.noteType, daNote.isSustainNote]);
	}
	...
	function noteMissPress(direction:Int = 1):Void //You pressed a key when there was no notes to press for this key
	{
		...
		callFunc('noteMissPress', [direction]);
		callOnLuas('noteMissPress', [direction]);
	}

	function opponentNoteHit(note:Note):Void
	{
		if(health > 0.05 && curSong == "double-trouble")
		{
            health -= 0.015;
		}
		...
		callFunc('opponentNoteHit', [notes.members.indexOf(note), Math.abs(note.noteData), note.noteType, note.isSustainNote]);
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			callFunc('goodNoteHit', [notes.members.indexOf(note), leData, leType, isSus]);
	...
	override function sectionHit()
	{
		...
		if (SONG.notes[curSection] != null)
		{
			...
			setVar('mustHitSection', SONG.notes[curSection].mustHitSection);
			setVar('altAnim', SONG.notes[curSection].altAnim);
			setVar('gfSection', SONG.notes[curSection].gfSection);
	...
	function setPlayStateVars()
	{
		...
		setVar('gfGroup', gfGroup);
		setVar('dadGroup', dadGroup);
		setVar('boyfriendGroup', boyfriendGroup);

		setVar('score', songScore);
		setVar('combo', combo);
		setVar('hits', songHits);
		setVar('misses', songMisses);
