...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		var sciana:BGSprite = new BGSprite('scana', -237, -125, 1, 1);
		sciana.setGraphicSize(2908, 2908);
		add(sciana);

		var fnf:BGSprite = new BGSprite('dywan', -598, 392, 1, 1);
		fnf.setGraphicSize(3120, 1560);
		add(fnf);

		var komp:BGSprite = new BGSprite('komputer', 50, 75, 1, 1);
		komp.setGraphicSize(Std.int(1.5 * komp.width));
		add(komp);

		var szafa:BGSprite = new BGSprite('szafa', -498, -125, 1, 1);
		szafa.setGraphicSize(Std.int(1.3 * szafa.width));
		add(szafa);

		var banhammer:BGSprite = new BGSprite('banhammer', 465, -100, 1, 1);
		banhammer.setGraphicSize(Std.int(1.4 * banhammer.width));
		add(banhammer);

		var pufa:BGSprite = new BGSprite('pufa', 800, 50, 1, 1);
		pufa.setGraphicSize(Std.int(0.75 * pufa.width));
		add(pufa);

		pow = new BGSprite('pow', 1000, 100, 1, 1, ['pow']);
		...
		var dark:BGSprite = new BGSprite('Darkness', 0, 0);
		dark.setGraphicSize(Std.int(dark.width * 2.0));
		add(dark);
		...
		blackScreen.setGraphicSize(Std.int(blackScreen.width * 2));
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				case 1:
					...
					#else
					createCoolText(['Korner\'s Funky Club Team']);
					#end
				...
				// credTextShit.screenCenter();
				case 9:
					addMoreText('It\'s not the fast food');
				case 11:
					addMoreText('You dummy\'s');
				case 15:
					deleteCoolText();
					case 16:
						addMoreText('WOJTI X KORNER IS A JOKE');
					case 20:
						addMoreText('FOR PEOPLE THAT DON\'T KNOW');
						case 22:
							deleteCoolText();
				case 24:
				...
				case 30:
					...
					case 31:
						addMoreText('Vs Korner\'s Funky Club'); // credTextShit.text += '\nFunkin';
