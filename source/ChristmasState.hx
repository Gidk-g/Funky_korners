...
sprite.drawCircle();
		...
		var bg:FlxSprite = new FlxSprite(-80).loadGraphic(Paths.image('MenuChristmas'));
        ...
        story = new FlxSprite(0,0).loadGraphic(Paths.image('Symbolch'));
		story.x += 275;
		story.y += 320;
		...
		WeekSaves.loadShit();
		if (FlxG.save.data.kornerchLock == "unlocked")
			{
		copied = new FlxSprite(0,0).loadGraphic(Paths.image('KORNERch'));
		copied.x += 342.5;
		copied.y += 360;
		...
		if (FlxG.save.data.wojtichLock == "unlocked")
			{
		cosplay = new FlxSprite(0,0).loadGraphic(Paths.image('WOJTIch'));
		cosplay.x += 502.5;
		cosplay.y += 360;
    ...
    function clickPlay() {
		FlxG.mouse.visible = false;
								...
								PlayState.storyPlaylist = ["mistletoe", "christmas"];
	...
	function clickPlayCopied() 
		{
			FlxG.mouse.visible = false;
		// Nevermind that's stupid lmao
		PlayState.storyPlaylist = ["mistletoe"];
...
function clickPlayCosplay() 
	{
		FlxG.mouse.visible = false;
	// Nevermind that's stupid lmao
	PlayState.storyPlaylist = ["christmas"];
	PlayState.isStoryMode = false;
	PlayState.seenCutscene = false;
