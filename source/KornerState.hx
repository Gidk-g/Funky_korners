...
class KornerState extends MusicBeatState
{
	public static var psychEngineVersion:String = 'v1'; //This is also used for Discord RPC
...
FlxG.mouse.useSystemCursor = true;
		...
		var bg:FlxSprite = new FlxSprite(-80).loadGraphic(Paths.image('MenuKorner'));
        ...
        story = new FlxSprite(0,0).loadGraphic(Paths.image('Symbol_12'));
		story.x += 275;
		story.y += 320;
		...
		bonus = new FlxSprite(0,0).loadGraphic(Paths.image('Symbol_123'));
		bonus.x += 275;
		bonus.y += 495;
		...
		WeekSaves.loadShit();
		if (FlxG.save.data.copiedLock == "unlocked")
			{
		copied = new FlxSprite(0,0).loadGraphic(Paths.image('kolzetogigachad'));
		copied.x += 342.5;
		copied.y += 360;
		...
		if (FlxG.save.data.copiedrLock == "unlocked")
			{
		copiedr = new FlxSprite(0,0).loadGraphic(Paths.image('franio'));
		copiedr.x += 342.5;
		copiedr.y += 535;
		...
		if (FlxG.save.data.cosplayLock == "unlocked")
			{
		cosplay = new FlxSprite(0,0).loadGraphic(Paths.image('kolzetogigachad'));
		cosplay.x += 502.5;
		cosplay.y += 360;
		...
		if (FlxG.save.data.luredLock == "unlocked")
			{
		lured = new FlxSprite(0,0).loadGraphic(Paths.image('kolzetogigachad'));
		lured.x += 662.5;
		lured.y += 360;
		...
		if(FlxG.save.data.cookieLock == "unlocked")
			{
			cookie = new FlxSprite(0,0).loadGraphic(Paths.image('powk'));
		cookie.x += 502.5;
		cookie.y += 535;
    ...
    function clickPlay() {
		if(!ClientPrefs.oldSongs)
			{
		FlxG.mouse.visible = false;
								// Nevermind that's stupid lmao
								PlayState.storyPlaylist = ["copied", "cosplay", "lured"];
							...
							else
								{
									FlxG.mouse.visible = false;
									// Nevermind that's stupid lmao
									PlayState.storyPlaylist = ["copied-old", "cosplay", "lured-old"];
									PlayState.isStoryMode = true;
									PlayState.seenCutscene = false;
	...
	function clickPlayCopied() 
		{
			...
			FlxG.mouse.visible = false;
		// Nevermind that's stupid lmao
		PlayState.storyPlaylist = ["copied"];
	...
	else
	{
		FlxG.mouse.visible = false;
		// Nevermind that's stupid lmao
		PlayState.storyPlaylist = ["copied-old"];
		PlayState.isStoryMode = false;
		PlayState.seenCutscene = false;
...
function clickPlayCopiedR() 
	{
		FlxG.mouse.visible = false;
	// Nevermind that's stupid lmao
	PlayState.storyPlaylist = ["copied-remix"];
	PlayState.isStoryMode = false;
	PlayState.seenCutscene = false;
...
function clickPlayCosplay() 
	{
		FlxG.mouse.visible = false;
	// Nevermind that's stupid lmao
	PlayState.storyPlaylist = ["cosplay"];
	PlayState.isStoryMode = false;
	PlayState.seenCutscene = false;
...
else
{
	FlxG.mouse.visible = false;
	// Nevermind that's stupid lmao
	PlayState.storyPlaylist = ["lured-old"];
	PlayState.isStoryMode = false;
	PlayState.seenCutscene = false;
...
function clickPlayPow() 
	{
		FlxG.mouse.visible = false;
	// Nevermind that's stupid lmao
	PlayState.storyPlaylist = ["cookie-beat"];
	PlayState.isStoryMode = false;
	PlayState.seenCutscene = false;
